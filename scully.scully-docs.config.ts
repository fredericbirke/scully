...
const defaultPostRenderers = [removeScripts, 'seoHrefOptimise', 'critters', copyToClipboard];
...
async function createConfig() {
  ...
  return {
    projectRoot: 'docs/scully-docs/src',
    projectName: 'scully-docs',
    outDir: 'dist/static/doc-sites',
    distFolder: 'dist/docs/scully-docs',
    spsModulePath: 'docs/scully-docs/src/app/app.sps.module.ts',
...
function getHeadings(content: string): [string, string][] {
  ...
  return content
    ...
    .filter(
      (line) =>
        line.startsWith('#') &&
        !exceptions.some((exception) => line.toLowerCase().includes(exception.toLowerCase().trim())),
    )
    .map((line) => {
