...
import { combineLatest, EMPTY, filter, forkJoin, Observable, throwError } from 'rxjs';
...
export class NavListService {
  scully = inject(ScullyRoutesService);
  /** return a tree structure for navigation */
  docTree$: Observable<DocTree> = forkJoin([
  ...
  languages$ = this.scully.available$.pipe(
    ...
    map((routes) =>
      routes
        .filter((r) => r.lang)
        .reduce((lang, cur) => {
          if (cur.lang && !lang.includes(cur.lang)) {
            lang.push(cur.lang);
          }
          return lang;
        }, [] as string[]),
...
function flattenTree(docTree: DocTree, list = [] as DocTree[]): DocTree[] {
  list.push(docTree);
