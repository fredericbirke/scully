...
import { initialStateDone, SCULLY_SCRIPT_ID, SCULLY_STATE_START, SCULLY_STATE_END } from '../utils/scully-consts';

type State = { [key: string]: any; } | undefined;
...
export class TransferStateService {
  ...
  private script = this.document.createElement('script');
  ...
  private nextUrl = this.router.events.pipe(
    ...
    switchMap((e: NavigationStart) =>
      this.router.events.pipe(
        filter((ev: any) => ev instanceof GuardsCheckEnd && ev.url === e.url),
        first()
      )
    ),
  ...
  private injectScript() {
    this.script = this.document.createElement('script'); // create fresh
  ...
  private saveState(newState: State) {
    if (isScullyRunning()) {
  ...
  private readFromIndex(url: string): Promise<object> {
