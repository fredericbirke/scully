...
export class ScullyContentComponent implements OnDestroy, OnInit {
  ...
  private async handlePage() {
    ...
    setTimeout(() => this.document.querySelectorAll('[href]').forEach((elm) => this.upgradeToRoutelink(elm as HTMLElement)), 10);
  ...
  private replaceContent(): void {
    ...
    let cur = findComments(parent, 'scullyContent-begin')[0] as ChildNode | null;
