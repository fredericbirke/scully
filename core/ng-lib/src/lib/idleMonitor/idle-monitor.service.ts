...
export class IdleMonitorService {
  ...
  private conf = inject(SCULLY_LIB_CONFIG);
  private tss = inject(TransferStateService);

  /** provide the default for missing conf parameters */
  private scullyLibConfig = Object.assign({}, ScullyDefaultSettings, this.conf)
  ...
  constructor(
    ...
    if (!this.scullyLibConfig.manualIdle && window && (this.scullyLibConfig.alwaysMonitor || isScullyRunning())) {
      ...
      this.router.events
        .pipe(
          filter((ev:any) => ev instanceof NavigationEnd && ev.urlAfterRedirects !== undefined),
