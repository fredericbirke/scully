...
import { BehaviorSubject, catchError, delayWhen, filter, firstValueFrom, from, interval, merge, Observable, of, shareReplay, switchMap, take, throttleTime, timer } from 'rxjs';
 ...
 */
export const launchedBrowser$ = of('').pipe(
...
switchMap(() => from(waitForIt(50))), switchMap(() => merge(obsBrowser(), launches)), 
/** use shareReplay so the browser will stay in memory during the lifetime of the program */
shareReplay({ refCount: false, bufferSize: 1 }), filter((e) => e !== undefined));
let usageCounter = 0;
export const launchedBrowser = async () => {
    if (++usageCounter > 500) {
        launches.next();
        usageCounter = 0;
    }
    return firstValueFrom(launchedBrowser$);
};
export let browser;
/** ICE relaunch puppeteer. */
// eslint-disable-next-line @typescript-eslint/no-unused-vars
export const reLaunch = (reason) => {
    //   if (reason) {
    //     logWarn(
    //       white(`
    // ========================================
...
function launchPuppeteerWithRetry(options, failedLaunches = 0) {
    let clear;
    const timeout = (millisecs) => new Promise((_, reject) => (clear = setTimeout(() => reject('timeout'), millisecs)));
    return Promise.race([
        ppt.launch(options),
