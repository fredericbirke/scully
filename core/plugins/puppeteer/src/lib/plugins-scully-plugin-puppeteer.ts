...
export const puppeteerRender = async (route: HandledRoute): Promise<string> => {
  ...
  try {
    ...
    if (scullyConfig.bareProject) {
      ...
      function registerRequest(request: { continue: () => void; }) {
        request.continue();
        requests.add(requests);
      }

      // eslint-disable-next-line no-inner-declarations
      function unRegisterRequest(request: any) {
        // request.continue();
      ...
      pageLoaded
        .pipe(
          ...
          filter(() => !!route?.config?.manualIdleCheck),
          take(1)
        )
        .subscribe({
          complete: () => {
    ...
    await page.exposeFunction('scullyPageReadyEvent', () => {
      resolve();
    });
    ...
    await page.evaluate(() => {
      ...
      document.body.setAttribute('scully-version', window['scullyVersion'] || 'unknown');
    });
...
const windowSet = (page: Page, name: string, value: {}) =>
