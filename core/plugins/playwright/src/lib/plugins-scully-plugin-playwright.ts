import { createFolderFor, HandledRoute, logError, logWarn, registerPlugin, scullyConfig, yellow, cliOptions } from '@scullyio/scully';
...
export const playwrightRenderer = async (route: HandledRoute): Promise<string> => {
  ...
  try {
    ...
    if (scullyConfig.ignoreResourceTypes && scullyConfig.ignoreResourceTypes.length > 0) {
      ...
      function checkIfRequestShouldBeIgnored(request: { resourceType: () => string; abort: () => void; continue: () => void; }) {
        if (scullyConfig && scullyConfig.ignoreResourceTypes && scullyConfig.ignoreResourceTypes.includes(request.resourceType())) {
          request.abort();
        } else {
          request.continue();
        }
    ...
    await page.evaluate(() => {
      ...
      const version=window['scullyVersion'] as string;

      document.body.setAttribute('scully-version',  version || 'unknown');
    });
    ...
    if (errorredPages.has(route.route) && (errorredPages.get(route.route)?? 0) > 2) {
