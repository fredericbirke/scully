import { loadConfig, cliOptions,logError, logWarn, white, yellow } from '@scullyio/scully';
...
const launch = async (pluginConfig: any): Promise<Browser> => {
  const browserType = pluginConfig.browser as any;
  const playrightBrowser = playwright[browserType] as any;
...
export const launchedBrowser$: Observable<Browser> = of('').pipe(
  ...
  filter((e: any): e is Browser => e !== undefined)
);
