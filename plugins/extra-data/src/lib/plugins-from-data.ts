 ...
 * @param options
 */
const dataRoutesPlugin = async (route: string, options: { data: any[]; }): Promise<HandledRoute[]> => {
  /**
  ...
  if (options.data) {
    ...
    return options.data.map((item: { data: any; title: any; }) => ({
...
const validator = async (options: { data: any[]; }) => {
  ...
  if (options.data) {
    if (!Array.isArray(options.data)) {
      ...
      if (!options.data.every((item: { title: any; data: any; }) => typeof item.title === 'string' && typeof item.data === 'string')) {
