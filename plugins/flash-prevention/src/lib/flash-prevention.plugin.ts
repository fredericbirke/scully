...
const MockRootAttrsBlacklist = [] as any[];
...
async function flashPreventionPlugin(html: string) {
  let newHtml = await createSecondAppRoot(html);
  newHtml = await addBitsToHead(newHtml);
  return newHtml;
}

async function createSecondAppRoot(html: string) {
  ...
  if (!openTagMatch || !closeTagMatch) {
    throw new Error(`Could not find ${appRootSelector} (or its closing tag) in html`);
  }
...
async function addBitsToHead(html: string) {
  const contentScript = `
...
function pushItemsToArray(src: string | string[], dest: string[]) {
  if (src) {
